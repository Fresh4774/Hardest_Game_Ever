...
function resetGame() {
  ...
  if (hardMode) {
    noteElement.innerHTML = `Click The Controller To Start!
        <p>If You Want A HARDER MODE THAN PRESS H!</p>`;
  } else {
    noteElement.innerHTML = `Click The Controller To Start!
        <p>If You Want A HARDER MODE THAN PRESS H!</p>`;
  }
...
function main(timestamp) {
  ...
  try {
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `You Lost A Ball, Press Space To Reset The Game!
        <p>
          Don't Rage Quit Just Go Back To Easy Mode By Pressing E!
        </p>`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
